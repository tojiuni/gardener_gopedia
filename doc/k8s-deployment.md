...
ssh -i <ssh-key-path> ubuntu@<k8s-master-ip> -p 22 \
...
# gardener-gopedia-xxx                1/1     Running   0   ← bot: vault 주입 없음
# gardener-gopedia-svc-xxx            2/2     Running   0   ← 2/2: app + vault-agent
```

`gardener-gopedia-svc` 는 Vault Agent Injector 사이드카가 추가되어 `2/2` 로 표시됩니다.
`gardener-gopedia` (bot) 은 Vault 주입 없이 `1/1` 로 실행됩니다.
